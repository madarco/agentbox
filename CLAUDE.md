...
- [`docs/workspaces-remote-hub-plan.md`](./docs/workspaces-remote-hub-plan.md) — **workspaces on a remote control box**: the 0.32 workspace layer is local-hub-only by construction (store on the hub's disk, folder paths validated on the hub, CLI pinned to the laptop hub). The plan moves the store to the hub that owns the boxes, makes a workspace repo-URL based with per-host folder mappings, keeps the manager on the PC (record on the hub, heartbeat for liveness), and drops the checkout dependency of push line counts and PR sync. Phases 2–4 gate on a real Hetzner control box.
- [`docs/workspaces-remote-hub-backlog.md`](./docs/workspaces-remote-hub-backlog.md) — the 0.31 features that still assume a local hub (`--model-auth` via hub, service-agent create routing, `git` routing for docker boxes, backup paths, …): documented with file refs, deliberately not fixed by that plan.
...
- [`docs/test-plan.md`](./docs/test-plan.md) — how to run the pre-release `pnpm e2e` suite, and the map from the old per-command checklist IDs (CLAW-006, TOOLS-004, …) to the scenario step, unit test or manual check that covers each now.
- [`docs/release-e2e-plan.md`](./docs/release-e2e-plan.md) — **the pre-release e2e run**: 8 real-life scenarios (install + bake, agent turn to a merged PR, lifecycle, services, host tools, openclaw, workspaces, interactive surfaces incl. the tray) run in parallel on every provider and a Linux host, against the packed CLI in a separate `HOME` on the same machine (`e2e/`, `pnpm e2e`; `/release-notes` refuses a stable tag without a green run). Its live backlog, including the product bugs it found, is [`docs/release-e2e-backlog.md`](./docs/release-e2e-backlog.md).
