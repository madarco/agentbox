...
- [`docs/local-adoption-plan.md`](./docs/local-adoption-plan.md) — the plan of record for **local adoption** (PC as thin client of the control box): making local `agentbox *` commands see control-box state — hub-created boxes in `agentbox ls` + auto-adopt on by-name miss, on-demand SSH-key download, seed-material custody push (untracked tar + env/secrets) so hub creates of PC-added projects are complete, and shared bake state (`prepared/<provider>.json` in custody). Four phases, none implemented yet.
