...
target the hosted control-plane unchanged (aligns with [`docs/control-box-plan.md`](./docs/control-box-plan.md)).
...
- [`docs/remote-docker-plan.md`](./docs/remote-docker-plan.md) — **remote-docker: how to finish the testing from the host.** The provider was built + verified from inside a box against a nested engine; a real Linux server and a macOS/OrbStack remote have NOT been tested. Step-by-step commands, what to watch for, gotchas already found.
...
- [`docs/control-box-plan.md`](./docs/control-box-plan.md) — the **single plan-of-record for the control box** (the full hub deployed on a persistent Hetzner VPS as an intermediary between the PC and cloud boxes). Covers the four implemented phases (SQLite hub core, custody store, full-hub Hetzner deploy + resident worker, PC-through-the-control-box), the live-verified goal scenario (web-UI create + host-off push), and the full backlog (notably the local-adoption gap). The older `control-plane-{roadmap,backlog,guide}.md` design docs were removed to avoid confusion; the public how-to guide is [`apps/web/content/docs/deployed-hub.mdx`](./apps/web/content/docs/deployed-hub.mdx) (published at `/docs/deployed-hub`). The hosted-service core, `agentbox control-plane` CLI, GitHub-App token leasing, Store seam, and poll-vs-block approvals are all described there.
- [`docs/local-adoption-plan.md`](./docs/local-adoption-plan.md) — **local adoption** (PC as thin client of the control box): local `agentbox *` commands see control-box state — hub-created boxes in `agentbox ls` + auto-adopt on by-name miss (`hub adopt`), on-demand SSH-key download from custody, seed-material custody push (untracked tar + env/secrets) so hub creates of PC-added projects are complete, and shared bake state (custody `prepared/<provider>.json`, fingerprint-match-wins). All four phases implemented 2026-07-17; the doc tracks what's live-verified vs. the remaining smoke pass.
