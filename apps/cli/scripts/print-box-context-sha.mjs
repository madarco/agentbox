...
// `--agent-install npm` prints the fingerprint of the NPM image variant.
// The install mode is part of the image's identity — the same context built with
// `AGENTBOX_AGENT_INSTALL=npm` is a different image — and the CLI asks for the
// folded sha (`agentInstallFingerprint` in pullOrBuild). CI must publish under
// that same tag or npm-mode users never get a pull hit. `native` is the identity.
const modeIdx = process.argv.indexOf('--agent-install');
const mode = modeIdx === -1 ? 'native' : process.argv[modeIdx + 1];
if (mode !== 'native' && mode !== 'npm') {
  process.stderr.write(`error: --agent-install must be 'native' or 'npm' (got '${mode}')\n`);
  process.exit(1);
}

const sha = await computeContextSha256(files);
process.stdout.write(agentInstallFingerprint(sha, mode) + '\n');
