...
1. **Resolve the provider flag from `$ARGUMENTS`:**
   ...
   - `docker` | `daytona` | `hetzner` | `vercel` | `e2b` | `digitalocean` | `aws` → pass `--provider $ARGUMENTS`
   - anything else → stop and tell the user the valid values are `docker`, `daytona`, `hetzner`, `vercel`, `e2b`, `digitalocean`, `aws`
