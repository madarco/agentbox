...
2. **Resolve the provider flag from `$ARGUMENTS`:** empty → none; `docker` | `daytona` | `hetzner` | `vercel` | `e2b` → `--provider $ARGUMENTS`; anything else → stop and report the valid values.
