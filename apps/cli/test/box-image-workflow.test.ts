 ...
 * `AGENTBOX_AGENT_INSTALL=npm` is a different image, and the CLI asks for a
 * correspondingly different tag (`agentInstallFingerprint`, folded in
 * `pullOrBuild`). CI used to build only the default (native) variant, so
 * `box.agentInstall: npm` users never got a pull hit — a wasted local build on
...
describe('box-image workflow', () => {
  it('publishes both install variants', () => {
    expect(workflow).toMatch(/agent-install:\s*\[native,\s*npm\]/);
  });
  ...
  it('actually builds the variant it names (the build-arg the Dockerfile reads)', () => {
    ...
    expect(workflow).toMatch(/AGENTBOX_AGENT_INSTALL=\$\{\{\s*matrix\.agent-install\s*\}\}/);
  });
  ...
  it('tags each variant with ITS OWN fingerprint', () => {
    ...
    expect(workflow).toMatch(
      /print-box-context-sha\.mjs --agent-install \$\{\{\s*matrix\.agent-install\s*\}\}/,
    );
  });
  ...
  it('lets only the native build claim `latest` and the version tag', () => {
    ...
    expect(workflow).toMatch(/if \[ "\$\{\{ matrix\.agent-install \}\}" = "native" \]/);
    expect(workflow).toMatch(/matrix\.agent-install == 'native'/);
  });
...
describe('the tag CI publishes is the tag the CLI resolves', () => {
  it('folds the install mode the same way on both sides', () => {
    ...
    expect(agentInstallFingerprint(base, 'native')).toBe(base);
    // `npm` is a distinct image, so a distinct tag.
    expect(agentInstallFingerprint(base, 'npm')).not.toBe(base);
    // Stable: CI and the CLI compute it independently, from the same helper.
    expect(agentInstallFingerprint(base, 'npm')).toBe(agentInstallFingerprint(base, 'npm'));
  });
});
