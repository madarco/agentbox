...
describe('CLI cloud-backend loader entry', () => {
  it('resolves a built-in cloud provider', async () => {
    ...
    expect(typeof (await cloudBackendLoader.resolveBackend('digitalocean'))?.exec).toBe('function');
  });

  it('returns null for docker and for non-built-in names', async () => {
