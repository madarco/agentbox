...
describe('lifecycle CLI surface', () => {
  ...
  it('install takes --force and --dry-run, plus `cmux`, `herdr`, `codex`, and `tray` subcommands', () => {
    expect(installCommand.name()).toBe('install');
    expect(installCommand.commands.map((c) => c.name())).toEqual(['cmux', 'herdr', 'codex', 'tray']);
