...
<Callout title="NOTE">
  Most access commands auto-unpause a paused box and start a stopped one before connecting —
  `shell`, `code`, `url`, `screen`, and `open` all wake the box for you. See [Core
  concepts](/docs/core-concepts) for what a box actually is, and [Run an agent](/docs/run-an-agent)
  for the agent session specifically.
</Callout>
...
<Callout title="TIP">
  Same detach chord as the agent: `Ctrl+a d`. See [Background and
  parallel](/docs/background-and-parallel) for juggling many boxes.
</Callout>
...
`agentbox screen [box]` opens the box's live desktop in your browser over noVNC. This is where you watch the in-box headed browser the agent drives, or any other GUI process. The per-box VNC password is embedded in the auto-connect URL, so the browser jumps straight to the desktop.
...
The desktop has a window manager and an icon dock along the bottom — a Terminal button, a Browser button, and one icon per open window, so you can switch between windows or open a shell without leaving the viewer. Launching the browser puts a progress window on the desktop until it is up (on an arm64 box the first launch also has to fetch its Chromium).

The CLI isn't the only way in — the same desktop is one click away from the `(VNC)` link in `agentbox ls`, the menu-bar app's **Open VNC**, and the hub UI's Access card, on every provider.
...
<Figure
  ...
  caption="noVNC view of a box's desktop: the in-box browser above the icon dock."
/>
...
<Figure
  ...
  caption="The dashboard: box sidebar on the left, actions for the selected box on the right, chord hints in the footer."
/>

<Callout title="TIP">
  The dashboard is the cockpit for parallel work — see [Background and
  parallel](/docs/background-and-parallel) for running many boxes at once, and [Run an
  agent](/docs/run-an-agent) for the agent session shown on the right. The full command reference is
  in the [CLI](/docs/cli) page.
</Callout>
...
<Callout type="warn" title="HEADS UP">
  The first attach to a fresh box downloads the IDE server (~70MB, keyed to your desktop IDE
  version). It's a one-time cost per box; later attaches are instant.
</Callout>

<Figure
  src="/screenshots/cursor.png"
  caption="Cursor attached to a box's /workspace via Dev Containers, with service-log terminal panels auto-opened"
/>
...
<Callout type="warn" title="open exposes SSH to the internet">
  `agentbox inbound open` makes port 22 publicly reachable. Access stays key-only — no passwords,
  and only the `vscode` user. Run `agentbox inbound mybox lock` when you no longer need it. Hetzner
  and DigitalOcean only.
</Callout>
