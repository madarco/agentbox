...
When copying a directory, heavy regenerable dirs (`.git`, `node_modules`, `bin`, `obj`, `packages`, `dist`, `.next`, `target`) are dropped by default; `exclude:` adds to that set. The host resolver blocks `..` traversal, denies `/proc|/sys|/dev|/etc/passwd|/etc/shadow`, caps each entry at 50 MiB **after excludes** (raise with `AGENTBOX_CARRY_MAX_BYTES`), and flags symlinks that escape `$HOME` and the project root. On `create`, `claude`, `codex`, and `opencode` the host prompts once — listing each src→dest with size, mode, and symlink warnings — before copying.
