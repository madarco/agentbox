...
**The approval is remembered per project.** Say yes once and later boxes for that project carry the same files without asking again; the grant is stored host-side in `~/.agentbox/projects/<hash>/carry.yaml` (never in the repo — a committed `carry:` block only _requests_ the copy, exactly as a `tools:` block does). It is keyed on the file **list**: change a `src`/`dest`, add or remove an entry, or change a `mode` — or have an entry start resolving through a symlink out of `$HOME` — and the next create asks again. A carried file whose **contents** change does not re-ask: you approved that path, and the file it points at is expected to change.
...
A remembered approval works there too — the preflight runs the same gate, so once a project's list is approved (from any surface) the web form and the tray card stop showing the question and go straight to the build.
