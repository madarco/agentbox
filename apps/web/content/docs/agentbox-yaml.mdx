...
`agentbox.yaml` lives at your workspace root and is read inside the box by the supervisor (`@agentbox/ctl`, shipped as `agentbox-ctl`). It declares **tasks** (one-shot setup units) and **services** (long-running processes) run under a DAG scheduler, plus a few host-side blocks (`carry`, `tools`, `defaults`, `ide`).
...
`carry` and `tools` are also declared at the top level but are host-applied and parsed separately — see [carry](#carry) and [tools](#tools) below. Service and task **names** must match `[A-Za-z0-9_-]+`.
...
<Callout title="TIP">
  Use the array form of `command` to avoid shell quoting; use the string form (`bash -c`) when you
  need shell features like pipes, `&&`, or sourcing env files.
</Callout>
...
<Callout type="warn" title="HEADS UP">
  Tasks re-run on every daemon start, not just at create. A non-idempotent task (an unguarded `git
  init`, a destructive migration) will fire repeatedly — declare `run_once:` (or guard it yourself).
  Prefer the `{check}` form for state that a checkpoint does not capture (containerized DB data),
  where a filesystem marker would desync.
</Callout>
...
<Callout title="TIP">
  Downstream units gated with `needs: [api]` start only once the probe reports `ready`, not merely
...
<Callout type="warn" title="HEADS UP">
  `as` must be `80` — it is the single container port AgentBox reserves for the published web URL.
...
<Callout type="warn" title="HEADS UP">
  A `needs` cycle or a reference to a non-existent unit fails validation — the daemon will not
  start, and on the host `agentbox create` aborts before doing any docker work.
</Callout>
...
<Callout title="TIP">
  Use `defaults` to make a repo "remember" its provider and checkpoint so teammates get the same box
...
| `user`        | Numeric uid that owns the file in-box. Leave unset (the default) and AgentBox resolves the box's `vscode` user itself — its uid is `1000` on some providers and assigned at bake time on vercel/e2b, so don't write `1000` meaning "the box user". `0` keeps it root-owned. |
...
**The approval is remembered per project.** Say yes once and later boxes for that project carry the same files without asking again; the grant is stored host-side in `~/.agentbox/projects/<hash>/carry.yaml` (never in the repo — a committed `carry:` block only _requests_ the copy, exactly as a `tools:` block does). It is keyed on the file **list**: change a `src`/`dest`, add or remove an entry, change a `mode`, an `exclude:` or a replacement rule — or have an entry start resolving through a symlink out of `$HOME` — and the next create asks again. A carried file whose **contents** change does not re-ask: you approved that path, and the file it points at is expected to change.

Two things to know about what a grant covers. It applies to **any provider**, so a list approved while creating a local Docker box is also carried, unasked, to a cloud box on Hetzner, E2B or Vercel — approve with that in mind, since those files leave your machine. And because the approval already came from a human, a **non-interactive** create (CI, a non-TTY) carries a granted list rather than failing for an explicit opt-in; the fail-loud rule below applies to a list this project has not approved.

`agentbox <agent> --carry ask` shows the table again. Answering **Skip** or **Cancel** there withdraws the standing approval, so the next create asks — that is how you revoke one (deleting `carry.yaml` works too).
...
# re-open an approval this project already gave
agentbox claude --carry ask
```

You can also set `AGENTBOX_CARRY_YES=1` or `AGENTBOX_CARRY=skip`; an explicit `--carry ask` beats both, since it is a request to review the list. Note that `-y`/`--yes` does **not** auto-approve carry — a non-TTY `-y` with an unapproved, non-empty list fails loud and asks for the explicit env var. `agentbox fork` is the exception: it sends carry by default (opt out with `agentbox fork --carry skip`).

### Approving carry outside the terminal

The same approval happens when you create a box from the **hub web UI** or the **macOS tray**: before anything is provisioned they ask the hub what the create needs approved (`POST /api/v1/projects/{id}/create-preflight`), and show the file table — each src→dest with size, mode, and symlink warnings — as a question you answer in the UI. The answers ride back with the create as `opts.promptAnswers`.

A remembered approval works there too — the preflight runs the same gate, so once a project's list is approved (from any surface) the web form and the tray card stop showing the question and go straight to the build. Both endpoints accept `carryAsk: true` to show the table again for a granted list, which is how a client offers a review-and-revoke.

A client that skips that step does not silently drop your files: carry has no safe default answer, so the create is refused with the same message the terminal gives you. A client that already has an answer sends it as a decision instead of an answer list — `opts.carryYes` to approve, `opts.carrySkip` to decline — which is how a scripted `agentbox create --carry-yes` or `--carry skip` gets through without a terminal. Answers are content-addressed to the question, so one collected against a `carry:` block that has since changed is refused rather than applied to the new one.

One case genuinely cannot ask: a **remote control box** has no checkout of your project and cannot read the files a `carry:` block names. It reports that (`unavailable`) and the UI tells you, instead of building a box quietly missing them. Create from the CLI on the machine that holds the files, which uploads the approved material with the create.

<Callout type="warn" title="HEADS UP">
  `carry` deliberately bypasses `.gitignore` and copies host files into the box. Treat the box as
  the untrusted side and only carry what the agent genuinely needs (credentials, env files). The
  per-entry cap is `box.cpMaxBytes` (default 100 MiB), the same limit `agentbox cp` enforces.
</Callout>

Mark credential entries `optional: true` so a box still comes up when a given file or agent isn't installed on the host. `optional` covers exactly one case — the source **does not exist**. It is not a blanket "ignore problems": once a file is there and approved, a copy that fails (unreadable source, a destination the box refuses, a mode that won't set) **fails the create** and names every entry that failed. A box that came up reporting success while quietly missing a credential was the worse outcome.

For how non-carry env files reach the box, see [environment](/docs/environment); for how git state is seeded, see [sync and git](/docs/sync-and-git) and [teleport a project](/docs/teleport-a-project).

## tools

Host CLIs the project's agents need — `terraform`, `aws`, `ntn`, `linear`. The box gets a shim; the **host** runs the real binary with its real credentials. Full guide: [Host tools](/docs/host-tools).
...
Or with per-tool options:

```yaml
tools:
  aws:
    bin: aws # host binary, when it differs from the name
    allow: ['^s3 ls', '^sts '] # run with no approval prompt
    deny: ['^s3 rm', '^iam '] # refused outright
    timeoutMs: 300000 # per-call budget (default 120000)
```

| Field       | Type     | Purpose                                                             |
| ----------- | -------- | ------------------------------------------------------------------- |
| `bin`       | string   | Host binary to run. Defaults to the tool name.                      |
| `allow`     | string[] | Regexes matched against the argv; matches skip the approval prompt. |
| `deny`      | string[] | Regexes refused outright, on top of the built-in credential guard.  |
| `timeoutMs` | int      | Per-call wall-clock budget. Host tools are non-interactive.         |

`allow` and `deny` patterns are compile-checked when the file is parsed, so a typo fails loudly instead of silently never matching.

<Callout type="warn" title="THIS BLOCK REQUESTS, IT DOES NOT GRANT">
  `agentbox.yaml` is committed, so a repo you cloned must not be able to reach your host credentials
  just by declaring a tool. AgentBox asks you once at create time and records the answer host-side;
  the relay reads only that record. Decline and the tools stay unavailable — the box is still
  created.
</Callout>
...
Three ways to apply substitutions:
...
**3. Clone-time (a spawned bot's identity files).** A rule-set named exactly `identity` is applied by `agentbox clone` to the files the box's agent declares as its identity — OpenClaw's `SOUL.md` and `IDENTITY.md` — so the new bot stops introducing itself with the source bot's name. It is applied once, on the host, to the copy on its way into the new box; your own files are never touched. Use `\b` word boundaries so a name does not rewrite words that merely contain it:
...
An OpenClaw bot can write this block itself — see [spawning a second bot](/docs/openclaw#spawning-a-second-bot).
...
<Callout title="TIP">
  Reach for `replaceEnvs`/`render` instead of hand-written `sed`: an `env` task that pins
  `BETTER_AUTH_URL`/`NEXT_PUBLIC_APP_URL` to `https://{{ AGENTBOX_BOX_HOST }}` becomes a one-liner,
  and the box and host browser then resolve the app at the same URL.
</Callout>
