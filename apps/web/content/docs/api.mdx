...
| `GET /boxes/{id}/prompts/stream` | Payload-carrying SSE of a box's approval prompts (the attach footer's channel). Emits `prompt-ask` (the full pending-approval payload), `prompt-resolved` (`{ id }`), `notice-set`/`notice-clear`, and `ping`. Distinct from `/api/events`, which carries refetch signals only (`data: {}`). |
...
titles, and `claudeActivity`/`codexActivity`. Cloud boxes also carry the fields
