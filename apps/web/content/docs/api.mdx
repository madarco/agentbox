...
| Method + path  | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `GET /system` | "What is running here, and do I need to re-bake?": `{ hub, build, deploy, providers, imageContextKeys }` — the hub version/channel/build source, the deploy record (when this machine is an exposed/deployed control box), each base provider's baked fingerprint + freshness (`baseStatus` `stale` = re-bake, with `baseStaleReason`), and the box-image build-context manifest (skills / agents / scripts baked in). Freshness is populated only on the in-process host topology (like `GET /providers?freshness=1`). Drives the web UI's **System & Build** page. |
