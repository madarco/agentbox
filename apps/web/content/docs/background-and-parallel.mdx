...
Where `drive` reads raw pixels, `agentbox agent` reports _what the agent is doing_, hook-driven and race-free. The state is one of `working | idle | waiting | end-plan | question | prompt | compacting | error | input-needed` — so you can watch a box flip from **working** to **idle**, or block until it needs you.
