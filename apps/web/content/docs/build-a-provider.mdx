...
<Callout title="v3 is a clean break">
  A provider built on SDK v2 is **not** loaded by this release. v2 exposed three
  per-agent staging helpers, so a v2 provider bakes a snapshot containing exactly
  claude, codex and opencode — on a host with any other agent it produces a base
  that silently lacks it. v3 replaces them with `stageAllAgentStatic`, which
  stages whatever agents the host actually has. Rebuild against `^3`, set
  `providerApiVersion: 3`, and replace the three `stage<Agent>StaticForUpload`
  calls with the single one.
</Callout>
