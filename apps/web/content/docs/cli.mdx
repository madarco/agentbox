...
`clone <box>` creates a new box from the source box's current workspace files and its `agentbox.yaml`, with a **fresh agent identity**: the agent's config volume and credential are deliberately not copied, so it onboards from scratch. The export and the create are one call to the hub (`POST /boxes/{id}/clone`), so the clone's workspace dir (`~/.agentbox/clones/<name>`, or `--into <dir>`) lands on the machine running the hub — the same machine the new box runs on. `-n/--name`, `-p/--provider` and `-y` behave as on `create`. Persistence is inherited from the source box (cloning an always-on service box gives an always-on clone); `--persistent` / `--no-persistent` override it. `.git` is not exported — the clone is a template; a git-backed second box on the same project is a plain `agentbox create`. There is no `--with-state`; to move a box's full state, use a [checkpoint](/docs/checkpoints-and-pausing). The clone is created without an agent — start one in it with `agentbox claude <clone>`.
