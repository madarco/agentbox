...
`install` is the first-run setup wizard (system check, pick a provider, log in, prepare its base image, install the host skill). `install cmux` pins a live `agentbox list` panel to the [cmux](https://cmux.com) sidebar dock (press `g` in the panel to toggle all-projects scope) — see [Access your box](/docs/access-your-box#cmux-a-sidebar-dock-panel-with-your-box-list). `doctor` diagnoses system and provider readiness. `prepare` builds base images or snapshots — omit `--provider` for status only.
