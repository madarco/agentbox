...
`config set`/`unset`/`edit` only write the `--global` or `--project` files (project is the default). The committed `defaults:` block is hand-edited — see [agentbox.yaml](/docs/agentbox-yaml) for its full schema. `engine.kind` is read by the engine detection itself, so it applies in every process that touches docker — the CLI, the hub, the relay.

{/* DIAGRAM /diagrams/configuration.png — nano-banana-pro, home-diagram style ref. Recipe + prompt in apps/web/images.md → Phase D. */}

<Figure
  src="/diagrams/configuration.png"
  caption="Config resolution order: the highest layer that sets a key wins; unset keys fall through to the built-in default."
/>
...
registered [Remote Docker](/docs/remote-docker) host rather than a provider:

```bash
agentbox config set box.provider docker:hub --global      # your control box's own engine
agentbox config set box.provider docker:buildbox          # a machine of yours
```

That form is shorthand for **two** keys, and it is stored as two:
...
So `agentbox config get box.provider` answers `remote-docker`, with the engine in
[`box.remoteDockerHost`](#box). Both spellings work in a hand-written config file —
...
Setting up a [control box](/docs/deployed-hub) does the first one for you when your
...
| `box.daytonaClass`                                                                                                         | string            | `container`        | Daytona [sandbox class](/docs/daytona#sandbox-class): `container` builds its base from the Dockerfile and runs in any region, so it works on every account. `linux-vm` gives a true pause (CPU + memory frozen, running processes survive) and a ~1 min base bake, but runs only in `us-east-1` — a dedicated region Daytona enables per organization, so set it only if yours is authorized. Changing it needs `agentbox prepare --provider daytona --force`. Daytona-only |
...
| `box.daytonaTimeoutMs`                                                                                                     | int               | `1500000` (25 min) | how long a box may sit idle before it's paused; `0` disables. The host enforces this (and holds the box open while the agent is working): Daytona's own timer is an _inactivity_ window that the relay's polling keeps resetting, so it only fires as a backstop when the relay isn't running. See [Daytona → idle boxes](/docs/daytona#idle-boxes-pause-themselves). Daytona-only                                                                                          |
...
| `box.remoteDockerHost`                                                                                                     | string            | empty              | default SSH destination whose Docker engine runs new boxes — an `~/.ssh/config` alias or `[user@]host[:port]`. Also where `box.provider: docker:<alias>` stores its host. Override per box with `agentbox docker:<host> …` or `--remote-host`. See [Remote Docker](/docs/remote-docker). remote-docker-only                                                                                                                                                                 |
...
| `box.persistent`                 | bool | `false`  | make the box **persistent** (always-on): never auto-paused, never idle-lapsed on a cloud provider, skipped by `agentbox prune`, confirmed before `destroy` even with `-y`, and started again by the relay after a host reboot. `--persistent` / `--no-persistent` at create. Refused on e2b and vercel, whose platform session cap the host can extend but not remove. See [persistent boxes](/docs/checkpoints-and-pausing#persistent-boxes)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `box.autoApproveSafeHostActions` | bool | `true`   | auto-approve the **safe** subset of host actions without a prompt: opening a PR, PR/review comments, merging a PR, re-running CI, checkpoints, calls to a granted [host tool](/docs/host-tools), file copy/download that stays inside the box project folder (non-secret), and opening a link from the box in your own browser (rate-limited to 2 per 30s and 10 per 10min per box — over that it asks, and an identical link repeated within 30s is dropped; see [browser & screen](/docs/browser-and-screen)). Uncontained/secret transfers, `gh pr checkout` and the destructive `gh` ops still prompt. `git push` is **not** governed by this key: an ordinary push to any branch runs silently and only a destructive push (a deletion, a force-push outside the box's own `agentbox/*` branch, `--mirror`/`--prune`, a redirected remote, an unrecognised flag) asks. Set `false` to prompt for every other host action. Each bypass is logged as a relay event. See [sync & git](/docs/sync-and-git) |
| `box.autoApproveHostActions`     | bool | `false`  | auto-approve **all** host-action confirms (the superset: destructive git pushes, cp, gh PR checkout and the destructive gh ops, checkpoint) for this box without a prompt; for unattended orchestration of trusted boxes. Each bypass is logged as a relay event. See [orchestration](/docs/background-and-parallel)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
An agent can also declare **settings of its own**, and those become config keys
in the same block. Claude declares two:
...
| `openclaw.modelAuth` / `pi.modelAuth` / `opencode.modelAuth` | list of `none`/`codex`             | `none`    | which host model-provider logins a new box of that agent is seeded with, comma-separated (`none` only on its own). `codex` copies your Codex (ChatGPT) OAuth login into the box and the agent imports it as its own OpenAI profile. `--model-auth` overrides per create. Only OpenClaw asks at create time — a service agent has no TUI to sign in through; for `pi` and `opencode` the flag or this key is the only way in. Note a seeded box cannot refresh a borrowed login itself — `agentbox credentials propagate --agent codex` is what renews it. `agentbox create` has no key of its own — an agentless box has no agent to default from, so its `--model-auth` is explicit or nothing. See [OpenClaw](/docs/openclaw#model-provider) |
...
[`agentbox agent add`](/docs/run-an-agent) gets real config keys the same way, with no
change to AgentBox — run `agentbox config list` to see what an agent declares.

<Callout type="warn" title="Renamed">
...
<Callout title="WHY">
  ...
  concepts](/docs/core-concepts) and [run an agent](/docs/run-an-agent).
</Callout>
...
`attach.cmuxStatus` surfaces a box's agent state in the [cmux](https://cmux.com) sidebar. While you're attached inside a cmux surface, AgentBox reflects the agent's live activity on the box's cmux workspace via its colour and description — blue/"working" while the agent runs, amber/"needs input" when it asks a question or is waiting, and the tint clears when idle. The workspace's original colour and description are restored when you detach. When you open several boxes from one project as tabs in the same workspace (`--attach-in tab`), AgentBox also **flags the individual tab** whose agent needs input via a cmux notification (tab badge + reorder + a desktop notification), so you can tell which box is waiting; the flag clears when you focus that tab. (cmux only renders its status _pills_ for workspaces running an agent it recognizes; a box runs the agent inside the container, so AgentBox drives the always-visible workspace colour/description and the per-tab highlight instead.) No effect outside cmux; set to `false` to disable.

`attach.herdrStatus` makes a box look like a _normal_ agent inside [Herdr](https://herdr.dev). While you're attached inside a Herdr pane, AgentBox reports the box agent's live activity to that pane (working / blocked / idle), so Herdr applies its native agent treatment — **including its own needs-input handling**. The one thing Herdr can't see is AgentBox's own host-relay **approval prompts** (git push, PR, checkpoint, …), so those get an explicit Herdr notification. No effect outside Herdr; set to `false` to disable. See [Herdr integration](/docs/integrations-herdr).

## manager

How a hub-run [manager](/docs/cli) session is carried and what ends it.

| Key                         | Type                               | Default  | Meaning                                                                  |
| --------------------------- | ---------------------------------- | -------- | ------------------------------------------------------------------------ |
| `manager.carrier`           | enum `auto`/`pty`/`tmux`           | `auto`   | which terminal carrier a `manager start` runs the session on             |
| `manager.lifetime`          | enum `leased`/`persistent`         | `leased` | whether a session is reaped when the client that leased it goes away     |
| `manager.leaseGraceSeconds` | int                                | `60`     | how long a leased session keeps running after its last holder disappears |
| `manager.scrollbackBytes`   | int                                | `524288` | recent output kept so a client attaching later gets a painted screen     |
| `manager.windowSize`        | enum `latest`/`smallest`/`largest` | `latest` | whose terminal size the session takes when several clients are attached  |
| `manager.submitDelayMs`     | int                                | `400`    | the pause between typing a message into a session and pressing Enter     |
| `manager.detachKey`         | string                             | `C-]`    | the chord that leaves `manager attach` without stopping the session      |

`manager.carrier: auto` runs the session on an AgentBox **pty host** — one process per session,
holding the agent's terminal with nothing in between — so scrollback, mouse, selection and a
modified Enter belong to the terminal you attach with. Where the optional terminal prebuild is
missing it falls back to a tmux session; `pty` refuses instead of falling back, and `tmux` keeps the
old carrier (whose keys and clipboard are limited by tmux options that can only be set for a whole
server, which is why the pty host exists).

`manager.lifetime` decides what a client leaving means. Under `leased`, a session a client holds a
lease on — the macOS tray does — is stopped once that client has stayed away for
`manager.leaseGraceSeconds`: quitting the app ends the terminals it opened, while an app restart or
update inside the window keeps them. A session nothing ever leased (one you started from a terminal)
is never reaped, and `agentbox manager pin <id>` exempts a single session. `persistent` exempts all
of them: only an explicit stop ends a session.

`manager.detachKey` takes a `C-<key>` spelling, or `none` to forward every byte and leave closing
the terminal as the only way out. Everything except the chord reaches the agent untouched.
...
Policy for the [host-tool proxy](/docs/host-tools). The _list_ of granted tools is not config — it is written by `agentbox tools add` and by approved in-box requests, and lives in a host-only grant file. These two keys are the policy around it.
...
agentbox config set --project tools.request.enabled false   # boxes may not ask
```

Whether a granted tool prompts on every call is controlled by `box.autoApproveSafeHostActions` (default `true` — granted tools run quietly, and every call is still audited).
...
A box created with `--persistent` (or under `box.persistent`) is exempt from all three: autopause never pauses it, and it is not counted against `maxRunningBoxes`. See [persistent boxes](/docs/checkpoints-and-pausing#persistent-boxes).
...
| `update.channel`                       | enum                                                     | `auto`                       | release channel `self-update` and `install app` follow: `auto` follows the installed build, `nightly` opts into [pre-release builds](/docs/nightly), `stable` opts back out. Nightly installs the newest build of _either_ channel, so a stable release supersedes the nightlies before it                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `update.registry`                      | string                                                   | `https://registry.npmjs.org` | npm registry [provider-plugin](/docs/build-a-provider) updates are resolved against, for `agentbox self-update` and `agentbox plugin update`. Point it at a mirror or a private registry when your plugins are not on the public one                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
