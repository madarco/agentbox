...
| `box.autoApproveSafeHostActions` | bool | `true`   | auto-approve the **safe** subset of host actions without a prompt: opening a PR, PR/review comments, re-running CI, pushing to the box's scratch or host-sanctioned branch, checkpoints, calls to a granted [host tool](/docs/host-tools), file copy/download that stays inside the box project folder (non-secret), and opening a link from the box in your host browser (rate-limited to 2 per 30s and 10 per 10min per box — over that it asks, and an identical link repeated within 30s is dropped). Uncontained/secret transfers, non-sanctioned-branch pushes, and PR merge/checkout still prompt. Set `false` to prompt for every host action. Each bypass is logged as a relay event. See [sync & git](/docs/sync-and-git) |
