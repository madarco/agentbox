...
| `relay.custodyMaxBodyBytes` | int (advanced) | `33554432` (32 MiB) | per-request body cap for the control box's custody uploads. Custody carries a project's untracked-files seed tar, which the relay's 1 MiB body cap is too small for; this applies only to custody, so every other route keeps the smaller cap. Raise it if a project's untracked seed is legitimately larger |
