...
| `box.sizeDaytona` / `box.sizeHetzner` / `box.sizeDigitalocean` / `box.sizeAws` / `box.sizeVercel` / `box.sizeE2b` | string (advanced) | empty | per-provider override of `box.size` |
...
| `box.awsRegion` | string | `us-east-1` | AWS region new boxes are created in; override per box with `--location`. AMIs are region-scoped, so this also decides which base AMI can boot. AWS-only |
| `box.awsSubnetId` | string (advanced) | empty | subnet new boxes launch into; empty = a public subnet of the account's default VPC. Set it when the account has no default VPC. AWS-only |
| `box.awsDiskGb` | int | `40` | root EBS volume size (GB) for new boxes (EC2's own 8 GB default is too small for the base image). AWS-only |
...
| `box.imageDocker` / `box.imageDaytona` / `box.imageHetzner` / `box.imageDigitalocean` / `box.imageAws` / `box.imageVercel` | string (advanced) | empty | per-provider override; the cloud ones are written by `agentbox prepare --provider <name>` |
