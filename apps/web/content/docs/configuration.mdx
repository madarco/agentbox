...
| `box.agentInstall`              | enum `native`/`npm`                | `native`  | how `agentbox prepare` installs Claude Code into the base image/snapshot. `npm` (`@anthropic-ai/claude-code`) is a fallback for cloud egress IPs the native installer's CDN 403s; bake-time only, so re-run `prepare` after changing it. Override per-run with `prepare --agent-install <mode>`                                                                                                                                                                                                                                                   |
| `box.claudeTui`                  | enum `default`/`fullscreen`/`auto` | `default` | terminal renderer Claude Code uses **inside a box**. Claude's `fullscreen` renderer repaints differentially and leaves stale characters in the blank areas of the screen over a network transport — visible while scrolling, cleared only by resizing the terminal. Boxes pin the classic renderer; set `fullscreen` to opt back in, or `auto` to let Claude decide. Applied via `/etc/agentbox/box.env`, so it takes effect on the box's next start                                                                                               |
...
where that agent actually has such a launch flag (OpenCode has none).
