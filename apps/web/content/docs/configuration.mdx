...
| `box.autoApproveSafeHostActions` | bool | `true`   | auto-approve the **safe** subset of host actions without a prompt: opening a PR, PR/review comments, re-running CI, pushing to the box's scratch or host-sanctioned branch, checkpoints, calls to a granted [host tool](/docs/host-tools), and file copy/download that stays inside the box project folder (non-secret). Uncontained/secret transfers, non-sanctioned-branch pushes, and PR merge/checkout still prompt. Set `false` to prompt for every host action. Each bypass is logged as a relay event. See [sync & git](/docs/sync-and-git) |
