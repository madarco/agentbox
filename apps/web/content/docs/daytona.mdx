...
Daytona resources are fixed **at snapshot-bake time** — the SDK rejects custom resources on the snapshot-resume path, so `--size` on `create` doesn't resize a snapshot-booted box. The size you bake at is pinned with `box.sizeDaytona` / `box.size` (`cpu-memory-disk` GB, e.g. `4-8-20`). The snapshot name carries a size suffix (`agentbox-base-<sha>-4-8-20`), so re-sized bakes don't collide; if you later `create --provider daytona --size 2-4-8` against a snapshot baked at a different size, the box still boots — AgentBox warns loudly that the size is fixed at bake time.

```bash
agentbox config set box.sizeDaytona 4-8-20
agentbox prepare --provider daytona --force
```

<Callout title="NOTE">`prepare` now bakes **on the hub**, and the hub bake does not yet read the `box.sizeDaytona` / `box.daytonaClass` pins — it bakes at the daytona defaults (`linux-vm`, default resources). Threading these bake knobs through the hub prepare path is a tracked follow-up in the [`/api/v1`](/docs/deployed-hub) consolidation; the per-invocation `prepare --size` flag was removed with it.</Callout>
...
- **Size is fixed at bake time** — snapshot-based creates inherit the snapshot's baked resources; pin `box.sizeDaytona` (a `create --size` that differs just prints a warning). The hub-routed bake does not yet honor that pin — see the sizing note above.
