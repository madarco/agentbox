...
An agent setting like [`claude.install: npm`](/docs/configuration#agent-settings) no longer forces the container fallback: the published base is agentless, so there is one image per build context and the setting reaches the derived agent tier below instead.
...
<Callout type="warn" title="HEADS UP">
  ...
  Daytona (Playwright + Google Chrome). Run `prepare` once.
</Callout>
