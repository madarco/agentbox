...
back into `~/.agentbox/<provider>-prepared.json`, so one bake leaves both machines current. Docker
and remote-docker always bake locally — their bases are local images, not portable snapshots. Passing
