...
The default matching _your CLI's_ version is deliberate: that parity is what makes the shared bake records line up (see the snapshot note above), so `agentbox self-update` followed by `agentbox hub update` keeps both sides together.
