...
<Callout type="info" title="A PC-created box's project isn't enough to Create from the UI">
  A box you spawned from your PC registers on the control box as a project you can **see and
  manage**, but its "Create box" button has no VPS checkout behind it — so use **Add project**
  (clone on the VPS) to originate new boxes from the browser.
</Callout>
