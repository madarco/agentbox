...
back into `~/.agentbox/<provider>-prepared.json`, so one bake leaves both machines current. The
control box's bake API carries only `--force` and `--claude-install`; the per-bake `--name`,
`--location`, and `--size` flags were removed while `prepare` moved behind the hub (a tracked
follow-up will thread the `box.size<Provider>` pins through the bake).
