...
laptop would spend minutes on a snapshot nothing boots. So `agentbox prepare` follows the same
routing as `create`:

```bash
agentbox prepare --provider e2b            # bakes on the control box, then adopts the record here
agentbox prepare --provider e2b --local    # bake on this machine instead
agentbox prepare --provider e2b --via-hub  # fail rather than fall back to a local bake
```

The bake's log streams to your terminal, and when it finishes the resulting bake record is pulled
back into `~/.agentbox/<provider>-prepared.json`, so one bake leaves both machines current. Passing
`--name`, `--location`, or `--size` keeps the bake local, since the control box's bake API doesn't
carry those.
