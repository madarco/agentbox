...
- **The direct IO plane.** `shell`, `attach`, `cp`, `download`, `code`, `open`, `url`, and `screen` talk to the box **from your laptop**, not through the hub. Moving them behind the hub (a uniform tunnel with hub-side SSH termination) is future work, explicitly out of scope for now. So these commands need your laptop up and reachable to the box. The exception is a copy a **box** starts (`agentbox-ctl cp`), which the hub brokers back to your machine — see [copying between a box and your files](#copying-between-a-box-and-your-files).
