...
<Callout type="info" title="Daytona / E2B / Hetzner need a base snapshot">Providers that boot from a baked base (e2b template, hetzner snapshot, daytona snapshot) read a `~/.agentbox/<provider>-prepared.json` to know the base id. The deploy **shares your local bake records** with the control box, and it adopts one automatically **when its build context matches** the control box's version — so a matching deploy needs no re-bake. If the control box runs a *different* AgentBox version than the one you baked with, the fingerprints differ and the shared record is (correctly) ignored; a create then fails with "run `agentbox prepare` first". Until the control box can self-bake, the workaround is to re-bake locally on the same version first, or copy the record up: `scp -i ~/.agentbox/control-plane/ssh/<stamp>/id_ed25519 ~/.agentbox/daytona-prepared.json root@<vps-ip>:/opt/agentbox/hub-data/`.</Callout>
