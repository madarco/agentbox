...
<Callout title="Nightly">E2B support is available in the **nightly** build of AgentBox.</Callout>
...
`agentbox e2b create|claude|codex|opencode|pi` is sugar for the same command with `--provider e2b`.
...
<Callout title="WHAT MAKES E2B DIFFERENT">
  ...
  steps, then snapshotting. **E2B is the only AgentBox cloud provider that builds the base directly
  from a Dockerfile** via the SDK's `Template.build()`. That means a deterministic, layer-cached,
...
E2B resources are set **at template-build time** — E2B rejects per-create resources, so `--size` on `create` doesn't resize a box. Bake the size you want with `agentbox prepare --provider e2b --size <cpu-memory>` (GB, e.g. `4-8` = 4 vCPU / 8 GB RAM), or pin it with `box.sizeE2b` / `box.size`:

```bash
agentbox prepare --provider e2b --size 4-8 --force
```

E2B has no disk knob, so a third `-disk` slot (`4-8-20`) is accepted but ignored with a warning. `prepare` folds the size into the skip-fast check, so a re-sized bake rebuilds while an unchanged one is reused. If you later `create --provider e2b --size 2-4` against a template baked at a different size, the box still boots — AgentBox warns loudly and points you at `prepare --provider e2b --size 2-4 --force`.
...
`agentbox e2b create|claude|codex|opencode|pi` all map to the same provider. From there, [run an agent](/docs/run-an-agent) and [access your box](/docs/access-your-box) as usual.
...
<Callout title="IN-BOX DOCKER">
  ...
  container runtime needs. The engine is baked into the prepare template and `dockerd` auto-launches
  on every create/resume, so a `postgres:16` `agentbox.yaml` service or ad-hoc `docker run` works
  with no opt-in. Pulled images carry over across pause/resume; the data root is wiped with the box
  on `destroy`.
</Callout>
...
<Callout type="warn" title="HEADS UP">
  `createSnapshot` pauses the source sandbox while capturing; the next op auto-resumes it. AgentBox
  self-heals a stale checkpoint: `create` probes liveness, prunes the dangling manifest, and falls
...
- **Template-level resources** — vCPU/RAM are baked at `Template.build()` time via `agentbox prepare --provider e2b --size 4-8`; E2B has no disk knob, and a per-create `--size` that differs from the baked size just warns.
