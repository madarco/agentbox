...
Locations are `nbg1` (Nuremberg), `fsn1` (Falkenstein), `hel1` (Helsinki), and `ash` (Ashburn, US). `--location` on a non-hetzner provider is ignored with a warning. The bake VPS location follows the `box.hetznerLocation` pin (the per-invocation `prepare --location` flag was removed when `prepare` moved behind the hub; honoring that pin through the hub bake is a tracked follow-up).
