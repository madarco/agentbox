...
| `GET /providers` | Sandbox providers with credential + baked status on this host: `{ id, label, configured, hasCredentials?, jobId?, reason? }`. `configured` = base image baked (usable for create); `hasCredentials` = credentials present (docker always); `jobId` = an in-flight bake, if any. |
