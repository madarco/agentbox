...
| `POST /boxes` | Create a box — returns `202 { jobId }`. Body: `{ projectId, agent, provider?, name?, prompt? }`. `agent: "none"` just creates the box without starting an agent (`prompt` ignored). `provider` defaults to `docker`; a cloud provider must be configured on the host (see `GET /providers`). |
