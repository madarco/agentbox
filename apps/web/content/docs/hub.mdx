...
- **Create boxes** — register a project folder and start a box (with an agent and
  ...
  panel and the box appears as `creating` → `running`. Pick agent **Empty — just
  create the box** to build a box without starting an agent (like `agentbox
  create`); attach later with `agentbox shell`.
