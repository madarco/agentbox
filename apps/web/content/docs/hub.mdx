...
`hub pull` needs a configured [control box](/docs/deployed-hub): it downloads a box the control box created into `~/.agentbox/boxes/<sandboxId>/ssh/`, so `agentbox attach` / port-forward / `cp` work exactly as for a box you created locally.
