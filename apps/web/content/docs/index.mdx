...
<Figure
  ...
  caption="agentbox ls — every box for the project at a glance: one per agent run, across local Docker and the cloud, each with its agent's live state."
/>
...
- **MacOS** or **Linux**
- An agent CLI — e.g. `claude`, `codex`, `opencode` or `pi`.
...
On a Mac you can also start from the [menu-bar app](https://github.com/madarco/agentbox/releases/download/tray-latest/AgentBox.dmg):
its setup wizard installs the CLI for you. You only need Node.js.
...
sandbox, the E2B template, etc.) so the first box starts fast, and finally whether
to make that provider the **default for new boxes** — say yes and a plain
`agentbox claude` goes there, with no `--provider` flag or provider prefix.

<Callout title="NOTE">
  You can run `agentbox install` again at any time to add more providers — each run only logs in and
  builds the base image for the providers you pick.
</Callout>
...
The prefix is only needed when the provider isn't your default. If you said yes to
"make it the default for new boxes" during `agentbox install`, plain `agentbox claude`
already goes there — see [`box.provider`](/docs/configuration#provider).
...
| `agentbox attach 1`                | Reattach to the running agent session on box 1 (claude / codex / opencode / pi). |
