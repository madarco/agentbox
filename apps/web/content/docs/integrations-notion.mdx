...
| `ntn api v1/<path>` | read | `GET`-only passthrough — `v1/search`, `v1/pages/<id>`, etc. Non-GET (`-X POST`, `-f`, `-F`, `--input`) is refused with exit 65. |
...
ntn whoami
ntn api v1/search -f "query=design doc"
