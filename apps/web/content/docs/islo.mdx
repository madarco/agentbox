...
<Callout type="info" title="Community provider">
  ...
  and the official [`@islo-labs/sdk`](https://www.npmjs.com/package/@islo-labs/sdk). Report bugs at
  the [issue tracker](https://github.com/zozo123/agentbox-provider-islo/issues) or
  **support@incredibuild.com**.
</Callout>

<Callout type="warn" title="Needs AgentBox 0.29.2 or earlier for now">
  The published `agentbox-provider-islo` targets **provider SDK v2**, and AgentBox 0.30.0 accepts
  only v4 — `agentbox plugin add` refuses it, and a copy registered before you upgraded is marked
  `!` by [`agentbox plugin list`](/docs/cli). Until an SDK v4 build is published, stay on AgentBox
  0.29.2 to use islo. Once one is, `agentbox self-update` picks it up on its own (or run `agentbox
  plugin update`). Track it at the [issue
  tracker](https://github.com/zozo123/agentbox-provider-islo/issues).
</Callout>
...
agentbox plugin list            # → islo … (SDK v4)
```
