...
The web hub and the macOS tray get the same link without the CLI: **Open Web** on a bot calls `GET /api/v1/boxes/{id}/web`, which answers `{ url, signInUrl }`. It is resolved when you click rather than carried on the box list, because reading the token is an exec into the box — and it re-resolves the URL at the same time, which matters on a box reached over an SSH forward, where the listed port can differ from the live one.
