...
- **macOS remotes work.** Docker Desktop, OrbStack and Colima are all found, because AgentBox runs `docker` in a login shell on the remote (which is why OrbStack's `~/.orbstack/bin` is on the PATH). A colima-driven Mac is a first-class remote — and the supported way to use colima at all, since local docker boxes on colima hit its macOS file-ownership sharing (see [local-docker](/docs/local-docker)).
