...
`agentbox remote-docker add` offers to do this for you when a control box is
configured, and a `hub setup` / `hub deploy` re-shares every host you had shared
before — a rebuilt control box doesn't lose them.
...
stays on. Pin something else any time with `agentbox config set box.provider

<name>`; `agentbox hub destroy` puts it back to `docker`.
