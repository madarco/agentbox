...
`--agent` is `claude` (default), `codex`, or `opencode` (OpenCode starts a fresh box since resume isn't supported). `--provider` selects `docker` (default), `daytona`, `hetzner`, or `vercel`. For `--session`, `--plan`, `--carry`, and the other flags, see [CLI commands](/docs/cli). Fork is also the `/agentbox` slash command inside a host Claude session.
