...
<Callout type="warn">
A box **cannot refresh a borrowed login by itself** — the provider rejects a
refresh from a different tool. The box works until the copied token expires,
and is renewed by `agentbox credentials propagate --agent codex`, which pushes
your host's refreshed login and re-runs the import. Boxes on their own login
(`agentbox codex`) are unaffected.
</Callout>
