...
<Callout type="warn" title="Needs AgentBox 0.29.2 or earlier for now">
  The published `agentbox-provider-sprites` targets **provider SDK v2**, and AgentBox 0.30.0 accepts
  ...
  0.29.2 to use sprites. Once one is, `agentbox self-update` picks it up on its own (or run
  `agentbox plugin update`). Track it at the [issue
  tracker](https://github.com/philippgerard/agentbox-provider-sprites/issues).
</Callout>
...
agentbox plugin list            # → sprites … (SDK v4)
```
