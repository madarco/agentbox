...
For an **SSH remote**, the box needs a passphrase-less key to push non-interactively (there's no ssh-agent in the box) — an HTTPS remote sidesteps this by using a token. Commit signing is enabled only when your signing key is usable without a passphrase; otherwise in-box commits are simply left unsigned so they never fail.
