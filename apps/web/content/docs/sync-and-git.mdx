...
The clone gets its own host workspace directory (`~/.agentbox/clones/<name>` by default, or
`--into <dir>`), exported from the source box with the same gitignore/exclude rules `download` uses.
That directory is a real project: `agentbox upload` pushes into the clone and `agentbox download`
pulls back out of it, like any other box.
...
<Callout title="NOTE">
  Today the clone is created **without an agent** — it is a box holding the workspace files and the
  `agentbox.yaml`, which is what a service box needs (its services are declared in that file). Start
  a coding agent in it with `agentbox claude <clone>` and it logs in as its own box.
</Callout>
