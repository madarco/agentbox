...
directory belongs to the box, not to your project folder. `--include-node-modules` keeps
`node_modules`; `--no-respect-gitignore` forces this mode even in a git workspace.
...
`agentbox sync [box]` is the other direction: it pushes your host workspace into a **live** box.
...
That directory is a real project: `agentbox sync` pushes into the clone and `agentbox download`
...
- [CLI reference](/docs/cli) — full flags for `download`, `sync`, `clone`, `cp`, `git`, and `relay`
