...
<Callout type="warn" title="Needs AgentBox 0.29.2 or earlier for now">
  The published `@tenkicloud/agentbox-provider` targets **provider SDK v2**, and AgentBox 0.30.0
  accepts only v4 — `agentbox plugin add` refuses it, and a copy registered before you upgraded is
  marked `!` by [`agentbox plugin list`](/docs/cli). Until an SDK v4 build is published, stay on
  AgentBox 0.29.2 to use tenki. Once one is, `agentbox self-update` picks it up on its own (or run
  `agentbox plugin update`). Track it at the [issue
  tracker](https://github.com/LuxorLabs/tenki-agentbox-provider/issues).
</Callout>
...
agentbox plugin list            # → tenki … (SDK v4)
```
