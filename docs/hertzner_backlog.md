...
- ✅ `packages/sandbox-hetzner/scripts/install-box.sh` — idempotent install script run on a fresh Ubuntu 24.04 VPS. Mirrors `packages/sandbox-docker/Dockerfile.box`:
  ...
  - Claude Code via native installer (Anthropic-canonical path) by default; `box.claudeInstall=npm` (or `prepare --claude-install npm`) swaps in `@anthropic-ai/claude-code` and symlinks it into `~/.local/bin/claude` — an opt-in escape hatch for the intermittent Cloudflare 403 the native CDN returns to Hetzner egress IPs. The install mode is folded into the base-snapshot fingerprint (via `claudeInstallFingerprint`), so switching modes re-bakes. Generic across all cloud/bake providers (hetzner/vercel/e2b/daytona/docker). Codex CLI (`@openai/codex`) + bubblewrap, OpenCode CLI (`opencode-ai`).
...
- **2026-07-10: compose + buildx CLI plugins now baked** (`docker-compose-v2` + `docker-buildx` apt packages in `install-box.sh`, mirroring `Dockerfile.box`) — user report: docs advertised Compose stacks but `docker compose` wasn't a command. **Snapshots baked before this need a re-`prepare --provider hetzner`.**
