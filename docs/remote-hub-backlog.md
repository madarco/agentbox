...
- [ ] `agentbox remote-docker add` bakes by calling the provider directly, so for a SHARED host it bakes from this machine instead of routing to the control box like `agentbox prepare` does. Same image, wrong streamer.
