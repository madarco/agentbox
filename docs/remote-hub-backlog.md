...
- [ ] `download.workspace` / `download.env` / `download.claude` still run on the control box with a cwd that no longer exists — the same defect `cp` and `tool.*` had. Both now broker through the host-reach channel, so each download kind is a routing line in the same branch.
