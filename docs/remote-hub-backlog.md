...
- [ ] remote-docker host aliases don't propagate to the control box. `agentbox docker add` writes `~/.agentbox/remote-docker-hosts.json` locally and nothing syncs it, so a hub-routed bake (`prepare --provider docker:<alias>`) falls back to the PC until the alias is registered on the control box by hand. Documented for now (remote-docker.mdx "Good to know", deployed-hub.mdx callout). If automated: call the hub's `POST /api/v1/hosts`, which probes ssh+docker before saving — never copy the registry file across, or `hubKnowsHost` goes true without the control box's key being authorized on the host and a working local bake is replaced by a failing remote one. The SSH-key half can't be automated.
