...
| 22 | **Hub/relay integration** | done | `kind: 'pty'` end to end: carrier dispatcher + `manager-pty.ts` + `pty-client.ts`, registration/status/stop/message/view, `runId`-verified detection, validators + OpenAPI, `spawnPtyHost` dep seam. Live: start → pty, message typed and submitted, survived a `hub restart`, stop reaped everything, and **Shift+Enter (`ESC[13;2u`) inserted a newline in claude with nothing rewriting it** — the thing tmux could not do. Fixed on the way: spec-on-fd-3 read empty (now a pipe), stop sent an empty token, and the hub's 0x0 control client resized the pty to 0 and killed the host. |
| 23 | **Lease, pin, config, janitor, fallback** | done | Seven `manager.*` keys resolved at the session's own folder, `lifetime`/`pin` honoured by the host's reaper, `POST …/{id}/pin` (record + live push), `apps/hub/lib/pty-janitor.ts`, configured detach chord. Live with an 8s grace: reaped after the holder left, **kept when the same client id returned inside the window**, pinned survived, unpinning reaped within a tick. |
| 24 | **Tray** | done | Pane execs `ptyAttach.command` + `--lease-id`; `ManagerTerminalView` (the Ctrl+J rewrite) deleted; neutral `terminalKey`; `TrayClient.leaseId` persisted in UserDefaults so a quit reaps and a relaunch re-claims. `swift build` + 105 tests green. **Open:** the live embed needs the control box updated too — its `ManagerRegistration` refuses `kind: 'pty'` until then. |
