/**
 * `agentbox codex` as a DESCRIPTOR, not a commander tree.
 *
 * The tree is built by `buildAgentCommand` in the CLI, which stays there
 * deliberately: its closure is the whole create/attach pipeline. What lives
 * here is the part that is genuinely Codex's — the wording that is not just
 * "codex" substituted into a shared sentence, plus the runtime it binds.
 */
import { resolveAgentSpec } from '@agentbox/sandbox-core';
import type { AgentCliSpec } from '@agentbox/cli-kit';
import { codexRuntime } from './runtime.js';

export const codexCliSpec: Omit<AgentCliSpec, 'attachWrapped'> = {
  id: 'codex',
  spec: resolveAgentSpec('codex'),
