/**
 * The AgentBox **Codex plugin** installer — everything `agentbox install codex`
 * does, minus the commander wiring.
 *
 * It lived in `apps/cli/src/commands/install-codex.ts`: ten exports, all of them
 * about codex's plugin marketplace and its `~/.codex/config.toml` enable table.
 * None of it is shared CLI machinery, so it sits with codex now and the command
 * file is the thin shim the other per-agent commands already are.
 *
 * `resolveDevRepoRoot` stays in the app and is passed IN — locating the user's
 * source checkout is the CLI's business, not codex's, and an agent package must
 * not reach up into `apps/cli`.
 */
...
export interface InstallCodexOptions {
  ...
  noDev?: boolean;
  /**
   * Locate the user's agentbox source checkout, or null when not in one.
   *
...
export async function installCodexPlugin(
  ...
  // the whole dev-checkout path (local marketplace + skill symlinks) while every
