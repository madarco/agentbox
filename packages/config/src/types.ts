...
export const BUILTIN_KEY_REGISTRY: readonly KeyDescriptor[] = [
  ...
  },
  {
    ...
    type: 'bool',
    description:
      "Auto-approve the SAFE subset of host actions without a prompt: opening a PR, PR/review comments, re-running CI, pushing to the box's scratch or host-sanctioned branch, checkpoints, calls to a granted host tool, file copy/download that stays inside the box project folder (non-secret), and opening a link from the box in your host browser (rate-limited to 2 per 30s and 10 per 10min per box; over that it asks). Uncontained or secret file transfers, non-sanctioned-branch pushes, and PR merge/checkout still prompt. On by default; set false to prompt for every host action (the pre-relax behavior). Superseded by box.autoApproveHostActions, which approves everything. Each auto-approval is recorded as a relay event.",
  },
  {
    key: 'box.persistent',
    type: 'bool',
  ...
  },
  {
    ...
    description:
      "Whether the local docker engine is offered on this machine: `auto` (default — docker and remote-docker are available until a control box is configured (`relay.controlPlaneUrl`), then gated off since a laptop-built docker box can't run with the laptop off), `thin` (force the gated behavior even without a configured control box), or `local` (keep docker on regardless of a control box). When gated, `create --provider docker` is refused, docker rows are dropped from the provider pickers / `doctor` / `prepare`, and docker boxes are shown as inactive in `ls`. With a control box configured, boxes are built by the control box and local docker on that machine is NOT supported: `local` still builds, but the box is owned by this machine's hub, so it is missing from `agentbox list` against the control box and every `agentbox git` command against it fails — every create/bake warns. `local` is the real fix only for `thin` with no control box. Exception: with `agentbox hub expose` the control box IS this machine, so docker stays on and nothing warns.",
  },
  {
    key: 'hub.gitAuth',
    type: 'enum',
