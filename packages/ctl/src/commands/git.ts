...
async function leaseAndPush(opts: CommonOptions, extra: string[]): Promise<number> {
  ...
  const cwd = opts.cwd ?? process.cwd();
  const lease = await postRpcAwait('git.lease-token', buildParams(opts, []), {
    errorPrefix: prefix,
  });
  if (lease.exitCode !== 0) {
...
export const gitCommand = new Command('git')
  ...
  .addCommand(
    new Command('push')
      ...
      .option(
        '--as <branch>',
        "with --host-only: destination branch name in the host repo (default: this box's branch name)",
      )
      .option(
        '--force',
      ...
      .action(async (args: string[], opts: PushOptions) => {
        if (opts.hostOnly && opts.remote) {
          process.stderr.write(
            'agentbox-ctl git push: --host-only does not use a remote; drop --remote\n',
          );
          process.exit(64);
        }
  ...
  .addCommand(
    new Command('pull')
      ...
      .action(async (args: string[], opts: CommonOptions & { ffOnly?: boolean }) => {
