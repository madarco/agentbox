#!/usr/bin/env bash
# AgentBox AWS EC2 base-image installer.
#
...
# `agentbox prepare --provider aws`. After this script completes we
...
if ! id vscode >/dev/null 2>&1; then
  # EC2's Canonical AMIs already create a UID 1000 user named `ubuntu`
  # (hetzner/DO images use `debian`/`ubuntu` too). If something owns UID
...
# claude-config volume mounted. EC2 has no shared volume — we
...
# `tar -xzf` of the host's ~/.claude in prepareAws extracts WITHOUT
...
# 24.04 EC2 instance we have to flip it ourselves.
#
...
if [ "${AGENTBOX_CLAUDE_INSTALL:-native}" = "npm" ]; then
  ...
  # intermittently 403s cloud-datacenter egress IPs (AWS among them) under
