 ...
 * `prepareAWS()` — same shape as the docker provider's runtime/docker
 * staging, but lighter (no Dockerfile build, just a flat tarball of files
 * to scp into /tmp).
 *
 * Lookup order for each file:
 *   1. The CLI's staged runtime tree: `<cliRoot>/runtime/aws/...`
...
/**
 * Locate the staged `runtime/aws/` tree. Two candidates:
 *
 *   1. Bundled CLI: the aws module is inlined into apps/cli's dist/, so
 *      `dirname(import.meta.url)` is `<cliRoot>/dist`; the staged runtime
 *      sits at `<cliRoot>/runtime/aws`.
 *   2. Workspace dev: this module's dist is at
 *      `packages/sandbox-aws/dist/`. There's no staged runtime there;
...
export function findStagedCliRuntimeRoot(): string | undefined {
  ...
  for (const c of candidates) {
    if (existsSync(resolve(c, 'aws', 'scripts', 'install-box.sh'))) return c;
  }
  return undefined;
}
...
export function candidatesFor(
  ...
  const monorepoRelative: Record<string, string[]> = {
    'install-box.sh': ['packages/sandbox-aws/scripts/install-box.sh'],
    ...
    'custom-system-CLAUDE.md': ['packages/sandbox-aws/scripts/custom-system-CLAUDE.md'],
  ...
  const cliRelative: Record<string, string[]> = {
    'install-box.sh': ['aws/scripts/install-box.sh'],
    'agentbox-ctl': ['aws/ctl.cjs'],
    'agentbox-vnc-start': ['aws/agentbox-vnc-start', 'docker/packages/sandbox-docker/scripts/agentbox-vnc-start'],
    'agentbox-dockerd-start': ['aws/agentbox-dockerd-start', 'docker/packages/sandbox-docker/scripts/agentbox-dockerd-start'],
    'agentbox-portless-trust': ['aws/agentbox-portless-trust', 'docker/packages/sandbox-docker/scripts/agentbox-portless-trust'],
    'agentbox-checkpoint-cleanup': ['aws/agentbox-checkpoint-cleanup', 'docker/packages/sandbox-docker/scripts/agentbox-checkpoint-cleanup'],
    'agentbox-open': ['aws/agentbox-open', 'docker/packages/sandbox-docker/scripts/agentbox-open'],
    'gh-shim': ['aws/gh-shim', 'docker/packages/sandbox-docker/scripts/gh-shim'],
    'git-shim': ['aws/git-shim', 'docker/packages/sandbox-docker/scripts/git-shim'],
    'ntn-shim': ['aws/ntn-shim', 'docker/packages/sandbox-docker/scripts/ntn-shim'],
    'linear-shim': ['aws/linear-shim', 'docker/packages/sandbox-docker/scripts/linear-shim'],
    'custom-system-CLAUDE.md': ['aws/custom-system-CLAUDE.md'],
    'claude-managed-settings.json': ['aws/claude-managed-settings.json', 'docker/packages/sandbox-docker/scripts/claude-managed-settings.json'],
    'agentbox-codex-hooks.json': ['aws/agentbox-codex-hooks.json', 'docker/packages/sandbox-docker/scripts/agentbox-codex-hooks.json'],
    'agentbox-setup-skill.md': ['aws/agentbox-setup-skill.md', 'docker/apps/cli/share/agentbox-setup/SKILL.md'],
  };
...
export function resolveRuntimeAssets(opts: {
  ...
  if (missing.length > 0) {
    ...
    throw new Error(
      `aws: could not resolve runtime assets — these files are needed to install on the prepare VPS:\n` +
        lines.join('\n') +
        `\n\nIf you are running from the monorepo, ensure \`pnpm -w build\` has run so packages/ctl/dist/bin.cjs exists. ` +
        `If you are running from a published CLI bundle, the runtime/aws tree should be staged automatically.`,
    );
  }
  return out;
}
