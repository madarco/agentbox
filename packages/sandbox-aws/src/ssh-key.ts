 ...
 * via cloud-init `users:` (NOT the AWS SSH-keys-import API, which
 ...
 *   ~/.agentbox/aws/prepare-<timestamp>/
...
// Hetzner backend). Re-exported here so the AWS-internal call sites
...
export async function mintPrepareKey(): Promise<MintedSshKey & { cleanup: () => Promise<void> }> {
  const root = resolve(homedirOrCwd(), '.agentbox', 'aws', `prepare-${Date.now().toString(36)}`);
