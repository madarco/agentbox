/**
 * Polling helpers — wait for a DigitalOcean Cloud resource to reach a desired
...
export async function pollUntil<T>(
  ...
  while (true) {
    ...
    if (Date.now() >= deadline) {
      throw new Error(`digitalocean: timed out waiting for ${label} after ${String(attempt)} attempts`);
    }
