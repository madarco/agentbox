 ...
 * `prepareDigitalOcean()` — same shape as the docker provider's runtime/docker
 * staging, but lighter (no Dockerfile build, just a flat tarball of files
 * to scp into /tmp).
 *
 * Lookup order for each file:
 *   1. The CLI's staged runtime tree: `<cliRoot>/runtime/digitalocean/...`
...
/**
 * Locate the staged `runtime/digitalocean/` tree. Two candidates:
 *
 *   1. Bundled CLI: the digitalocean module is inlined into apps/cli's dist/, so
 *      `dirname(import.meta.url)` is `<cliRoot>/dist`; the staged runtime
 *      sits at `<cliRoot>/runtime/digitalocean`.
 *   2. Workspace dev: this module's dist is at
 *      `packages/sandbox-digitalocean/dist/`. There's no staged runtime there;
...
export function candidatesFor(
  ...
  const monorepoRelative: Record<string, string[]> = {
    'install-box.sh': ['packages/sandbox-digitalocean/scripts/install-box.sh'],
  ...
  const cliRelative: Record<string, string[]> = {
    ...
    'ntn-shim': ['digitalocean/ntn-shim', 'docker/packages/sandbox-docker/scripts/ntn-shim'],
    'linear-shim': ['digitalocean/linear-shim', 'docker/packages/sandbox-docker/scripts/linear-shim'],
  ...
  if (missing.length > 0) {
    ...
    throw new Error(
      `digitalocean: could not resolve runtime assets — these files are needed to install on the prepare VPS:\n` +
        ...
        `If you are running from a published CLI bundle, the runtime/digitalocean tree should be staged automatically.`,
    );
  }
  return out;
}
