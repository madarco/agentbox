/**
 * Incremental host -> box seed of Claude Code's *dynamic* config: the global
 * `~/.claude/workflows/` scripts and the current project's `memory/` tree.
 * Unlike the static config (plugins/skills/settings) that the cloud path bakes
 * into the prepare-time snapshot, these change between runs and must be
 * refreshed per-box at create — like credentials.
 *
 * The pieces here are provider-neutral and pure where possible so both the
 * docker volume sync and the cloud `seedDynamicConfig` reuse them:
 *
 ...
 *     whose members are box-absolute paths, so the box extracts with `-C /`.
 *
...
import { BOX_CLAUDE_PROJECT_DIR, resolveClaudeMemoryDir } from './host-stage.js';
