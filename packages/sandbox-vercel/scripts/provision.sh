...
# agent-browser is BOX RUNTIME, not an agent -- the Chrome runtime libs below
# and AGENT_BROWSER_EXECUTABLE_PATH in the login shim both depend on it. It
# used to share one `npm install -g` line with codex and opencode; dropping
# that whole line for an agentless base would silently break the browser.
npm install -g agent-browser 2>&1 | tail -3 || \
  echo "provision.sh: agent-browser npm install failed (continuing)"
...
if [ ! -x /opt/google/chrome/chrome ]; then
  echo "provision.sh: Google Chrome install failed (/opt/google/chrome/chrome missing)" >&2
  exit 70
fi
...
if printf '%s\n' "$LDD_OUT" | grep -q 'not found'; then
  echo "provision.sh: Chrome has unresolved shared libs:" >&2
...
# sources.
step "relay shims (gh + git + tool)"
install -m 0755 /tmp/agentbox-gh-shim     /usr/local/bin/gh
install -m 0755 /tmp/agentbox-git-shim    /usr/local/bin/git
install -m 0755 /tmp/agentbox-tool-shim  /usr/local/bin/agentbox-tool-shim
done_ "relay shims (gh + git + tool)"
